...
partitioned_delete()
{
    ...
    if [[ "$policy" == "daily" ]] && [[ -n "$dailyPolicyDays" ]]; then
        ...
        bq query --nouse_legacy_sql --format=csv --max_rows=100000 'select DISTINCT DATE('${partColumn}') as day from `'${project}'.'${dataset}'.'${table}'` WHERE DATE('${partColumn}') <= CURRENT_DATE() group by 1 order by 1 desc ' | grep -v day | sed '/^[[:space:]]*$/d' > /tmp/temppartitionedtable_${filenamedate}.txt
