...
def get_gcp_project():
    ...
    to DEV otherwise.
    """
    return getenv('NEWSID_GCP_PROJECT', 'newscorp-newsid-dev')
...
def get_gcs_bucket():
    environ = getenv('NEWSID_ENV', 'dev')
    return "newscorp-newsid-bq-evictions-{}".format(environ)
...
def extract_to_gcs(dataset_id, table_id):
    ...
    pypath = getenv('NEWSID_GCP_EVICTION_PYTHON2_PATH', '/usr/lib/python2.7/bin/python')
    policylocation = getenv('NEWSID_GCP_EVICTION_POLICY_LOCATION', '/eviction/policies')
    workercount = getenv('NEWSID_GCP_DF_WORKER_COUNT', '40')
    maxworkercount = getenv('NEWSID_GCP_DF_MAX_WORKER_COUNT', '100')
    statuslocation = getenv('NEWSID_GCP_EVICTION_STATUS_LOCATION', '/eviction/logs')
...
def delete_table(dataset_id, table_id, eviction_policy, evict=True):
    ...
    if evict:
        if extract_to_gcs(dataset_id, table_id, eviction_policy):
            ...
            if oldBackupExists:
                log_msg("Table {} *has been* deleted but not evicted as backup with the same name already exists".format(
                    table_id
                    )
                )
            else:
...
def delete_table_with_retention(dataset_id, table_id, retention_period, eviction_policy, dry_run, evict=True):
    ...
    """
    Gets a list of tables to be deleted from BQ that was identified from the retention policy set on table level.
    """
    project = get_gcp_project()
    downstream_buffer = getenv('NEWSID_GCP_BQ_DOWNSTREAM_BUFFER', '3')
    # deletes the table
    statuslocation = getenv('NEWSID_GCP_EVICTION_LOG_LOCATION', '/eviction/logs')
    policylocation = getenv('NEWSID_GCP_EVICTION_POLICY_LOCATION', '/eviction/policies')
    table_id = table_id
    ...
    for line in lines:
        ...
        if (table_status == 'backuponlyNormal') or (table_status == 'true'):   
            ...
            if extract_to_gcs(dataset_id, table_id, eviction_policy):
                if oldBackupExists:
                    log_msg(
                        "Table {} *was not* evicted as backup with the same name already exists".format(
                            table_id
                        )
                    )
                else:
                    log_msg(
                        "Table {} *has been* evicted and deleted.".format(
                            table_id
                        )
                    )
