...
def delete_table(dataset_id, table_id, evict=True):
    """
    ...
    if evict:
        if extract_to_gcs(dataset_id, table_id):
        ...
        log_msg("Table {} *has been* deleted without being evicted.".format(
                table_id
            )
        )

def delete_table_with_retention(dataset_id, table_id, retention_period, dry_run, evict=True):
    """
    This method is used across all policies when deleting a table is
    required so that it can be done consistently.
    """
    ...
    eviction_with_retention_bash = '''bash {policylocation}/bqtable_deletion_policy.sh {dry_run} {project} {downstream_buffer} {dataset_id} {table_id} {retention_period} {statuslocation}'''
    ...
    try:
      ...
      return False
    for line in lines:
        deleted_table_name, deleted_table_status = line.split(':')
        deleted_table_name=deleted_table_name.strip()
        deleted_table_status=deleted_table_status.strip()
        if deleted_table_status == 'false':
            # Feedback
            log_msg(
        ...
        elif deleted_table_status == 'true':   
            log_msg(
                "Table Sucessfully deleted without being evicted {}:{}.{}".format(
                    get_gcp_project(),
                    dataset_id,
                    deleted_table_name
                )
            )
        elif deleted_table_status == 'skipped':   
            log_msg(
                "Table Skipped from deleteion as the table is modified recently, computed using downstream_buffer {}:{}.{}".format(
                    get_gcp_project(),
                    dataset_id,
                    deleted_table_name
                )
            )
        elif deleted_table_status == 'dry_run':   
            log_msg(
        ...
        elif deleted_table_status == 'no_tables_to_delete':   
            log_msg(
...
def evict_and_delete_table_with_retention(dataset_id, table_id, retention_period, dry_run, evict=True):
    """
    This method is used across all policies when deleting a table is
    required so that it can be done consistently.
    """
    ...
    table_id = table_id
    retention_period = retention_period
    eviction_with_retention_bash = '''bash {policylocation}/find_tables_for_deletion_with_retention.sh {dry_run} {project} {downstream_buffer} {dataset_id} {table_id} {retention_period} {statuslocation}'''
